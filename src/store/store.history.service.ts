...
import { StoreHistory } from "./entities/store.history.entity";
...
export class StoreHistoryService {
  ...
  async findOne(id: string): Promise<StoreHistory | null> {
    try {
  ...
  async update(id: string, updateStoreHistoryDto: any) {
    try {
      const history = await this.storeHistoryModel.findByIdAndUpdate(id, { $set: updateStoreHistoryDto }, { new: true })
      return history;
    } catch (error) {
